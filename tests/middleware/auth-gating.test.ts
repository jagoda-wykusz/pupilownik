...
import { createAuthenticatedCookieHeader, createInvalidCookieHeader } from "../helpers/session";
...
// mocked — that is the named anti-pattern). Proves the gate on /dashboard:
//   - closes for an unauthenticated request (no owner data leaks: next() never runs),
//   - opens for a genuine captured session,
//   - stays closed for a present-but-invalid session (a token ≠ authorization).
describe("auth gating (middleware) — /dashboard", () => {
  it("no session cookie → redirects to signin and never reaches the route", async () => {
    const { response, nextCalled, locals } = await runMiddleware({ pathname: "/dashboard" });

    expect(response.status).toBe(302);
    expect(response.headers.get("Location")).toBe("/auth/signin");
    // next() never ran → the dashboard handler never executed → no owner data served.
  ...
  it("present-but-invalid session cookie → still redirects to signin", async () => {
    const { response, nextCalled, locals } = await runMiddleware({
      pathname: "/dashboard",
      cookieHeader: createInvalidCookieHeader(),
    });
