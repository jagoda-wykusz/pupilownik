...
- [ ] 2.1 Migrations apply cleanly: `npm run db:reset`
- [ ] 2.2 Reveal tests pass, including both directions: `npm run test -- tests/rls/reveal-instructions.test.ts`
- [ ] 2.3 View unit tests pass: `npm run test -- tests/unit/invite-view.test.ts`
- [ ] 2.4 Claim route tests pass unchanged: `npm run test -- tests/api/invite-claim.test.ts`
- [ ] 2.5 Full suite green: `npm run test`
- [ ] 2.6 Types check: `npx astro check`
- [ ] 2.7 Lint passes: `npm run lint`

#### Manual

- [ ] 2.8 Widening fails closed when the revoked branch is moved before the digest gate
- [ ] 2.9 Holder sees the called-off card with no trip content in the response
- [ ] 2.10 Cookie-less window is byte-identical to a random token
- [ ] 2.11 The generic dead-link card no longer mentions asking for a new link
...
- [ ] 4.1 Full suite green: `npm run test`
- [ ] 4.2 Lint and formatting pass: `npm run lint`
- [ ] 4.3 Build succeeds: `npm run build`
- [ ] 4.4 No stale forward references: `grep -rn "S-06" supabase/migrations/ docs/ context/foundation/`

#### Manual

- [ ] 4.5 The two new registry rows and `regenerate_period_token`'s row describe live behaviour
- [ ] 4.6 Rule 4 reads as one rule with two bounded widenings
- [ ] 4.7 The live catalog's `revoked_at` column comment matches the migration
