...
3. ~~**Wyścig przy zwalnianiu terminu**~~ — **rozstrzygnięte 2026-09-14, zamknięte kodem.** `release_slot` nie miał optymistycznej blokady: zwolniony termin wraca do stanu `claimed_by_name is null`, czyli dokładnie tego, w który wpisuje się `claim_slots`, a strażnikiem było `claimed_at is not null` — test na to, że termin trzyma KTOKOLWIEK, nigdy na to, że trzyma go osoba, którą właściciel miał przed oczami. Sekwencja „karta A pokazuje Anię → karta B zwalnia → Basia zajmuje przez wciąż żywy link → karta A zwalnia na nieaktualnym widoku" po cichu kasowała świeży zapis Basi, a jej odsłonięcie znikało bez słowa. Pierwotne uzasadnienie odłożenia („potrzebne dwie równoczesne sesje właściciela, a slot i tak kończy jako wolny") nie przetrwało kontaktu z wdrożonym ekranem: wystarczy jedna osoba z dwiema kartami, bo wyspa przeładowuje stronę po sukcesie i unieważnia każdą inną; a „slot i tak kończy jako wolny" jest prawdą o slocie i nieprawdą o produkcie — właściciel prosił o zwolnienie terminu ANI, nie Basi. Zamknięte argumentem `p_expected_claimed_at` wpiętym w `WHERE` (`supabase/migrations/20260914150000_release_slot_expected_claimed_at.sql`), dokładnie tak, jak ta notatka zapowiadała: strona odsyła `claimed_at`, który wyrenderowała, niezgodność podnosi PT412, trasa odpowiada 409 i zdaniem, które kieruje właściciela na odświeżenie zamiast na ponowną próbę. Ten sam kształt co `p_expected_updated_at` z S-09. Pokryte przez `tests/rls/release-slot.test.ts` (§ „optimistic concurrency"), `tests/api/release-slot.test.ts` (§ „a term that changed hands after the page was rendered") i test komponentu rozróżniający 409 od 404.
