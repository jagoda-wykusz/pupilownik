...
- **Outcome:** właściciel może zmienić dane zwierzęcia i jego instrukcje opieki — w tym w trakcie trwającego wyjazdu, bo opiekun czyta instrukcje na żywo — oraz usunąć zwierzę, którego nie obejmuje żaden nieodwołany wyjazd. TRZY odmowy są częścią efektu, nie jego brakiem: przełączenie `is_sensitive` na istniejącym wierszu jest zablokowane, gdy na żywym wyjeździe ktoś zajął już termin; usunięcie jest zablokowane, dopóki jakikolwiek nieodwołany wyjazd obejmuje zwierzę; zapis z formularza otwartego przed cudzym zapisem jest odrzucany (`PT412`), zamiast po cichu skasować wiersze, których ten formularz nie znał. Wszystkie trzy są predykatami SQL, bo granty tabelaryczne pozwalają właścicielowi pisać wprost przez PostgREST. **Trzecia doszła po przeglądzie implementacji** — pierwotny zapis mówił o dwóch i był prawdziwy w dniu planowania; zdanie poprawione przy zamknięciu, zgodnie z `lessons.md`, żeby `## Done` opisywał to, co wylądowało, a nie to, co zapowiadano.
...
- **S-09: właściciel może zmienić dane zwierzęcia i jego instrukcje opieki — w tym w trakcie trwającego wyjazdu, bo opiekun czyta instrukcje na żywo — oraz usunąć zwierzę, którego nie obejmuje żaden nieodwołany wyjazd. TRZY odmowy są częścią efektu, nie jego brakiem: przełączenie `is_sensitive` na istniejącym wierszu jest zablokowane, gdy na żywym wyjeździe ktoś zajął już termin; usunięcie jest zablokowane, dopóki jakikolwiek nieodwołany wyjazd obejmuje zwierzę; zapis z formularza otwartego przed cudzym zapisem jest odrzucany (`PT412`), zamiast po cichu skasować wiersze, których ten formularz nie znał. Wszystkie trzy są predykatami SQL, bo granty tabelaryczne pozwalają właścicielowi pisać wprost przez PostgREST. **Trzecia doszła po przeglądzie implementacji** — pierwotny zapis mówił o dwóch i był prawdziwy w dniu planowania; zdanie poprawione przy zamknięciu, zgodnie z `lessons.md`, żeby `## Done` opisywał to, co wylądowało, a nie to, co zapowiadano.** — Archived 2026-09-14 → `context/archive/2026-09-13-pet-edit-and-delete/`. Lesson: —. **Trzy rzeczy, których `Outcome` nie zapowiadał, a które wylądowały:** (1) `/pets` i `/pets/new` przeszły z `bg-cosmic` na tokeny, a `AppBar` dostał nawigację między sekcjami — poza planem, po pierwszym przejściu manualnym (`0c3e9f0`, zapisane w Deviations); (2) usunięto `auth/FormField` i `auth/PasswordToggle`, bo `AddPetForm` był ich ostatnim konsumentem; (3) przegląd implementacji zamknął dwie zmierzone dziury współbieżności — brak locka w `delete_pet` (kaskada kasowała świeżo utworzone powiązanie) i brak tokenu wersji przy PUT (zapis ze starego formularza kasował cudze wiersze z kodem 200). Obie z pomiarem i mutacją, opisane w `context/archive/2026-09-13-pet-edit-and-delete/reviews/impl-review.md`. **Otwarte:** suite integracyjny mignął czerwonym 2 razy na 5 przebiegów (w izolacji 3/3 zielone); kształt błędu wskazuje na transport lokalnego stacka, przyczyna NIEZNANA.
